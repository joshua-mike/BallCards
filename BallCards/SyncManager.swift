...
class SyncManager {
	...
	func performFullSync(completion: @escaping (Bool, Error?) -> Void) {
		guard !syncInProgress else {
			completion(false, NSError(domain: "SportsCardLog", code: 100, userInfo: [NSLocalizedDescriptionKey: "Sync already in progress"]))
			return
		}
	...
	private func downloadCard(document: QueryDocumentSnapshot, completion: @escaping (Bool, Error?) -> Void) {
		...
		do {
			...
			if let estimatedValue = data["estimated_value"] as? NSNumber {
				card.estimated_value = estimatedValue.decimalValue
			}
...
// MARK: - Card Fetch Request Extension
extension Card {
	static var fetchRequest: NSFetchRequest<Card> {
